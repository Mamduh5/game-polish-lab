...
interface SaveMessage {
  command: "saveAndApply" | "setupBridge" | "applyAsset" | "markResult" | "undoLastApply" | "writeLiveStyle";
  surfaceType: VisualSurfaceType;
  presetName: string;
...
type FarmSlotRuntimeEditorState = "game_not_running" | "live_file_waiting" | "connected" | "style_applied" | "error";
...
async function writeFarmSlotLiveStyle(folder: vscode.WorkspaceFolder, message: SaveMessage): Promise<SaveResultMessage> {
  ...
  const state = await resolveFarmSlotRuntimeEditorState(folder);
  return {
    command: "saveResult",
    ok: true,
    surfaceType: "slot_card",
...
async function resolveFarmSlotRuntimeEditorState(folder: vscode.WorkspaceFolder): Promise<FarmSlotRuntimeEditorState> {
  try {
    const state = await getIdleMonsterFarmFarmSlotAdapterState(folder);
    if (!state.connection.connected) {
      return "live_file_waiting";
    }
    const served = await canReadLiveStyleFromDevServer();
...
function canReadLiveStyleFromDevServer(): Promise<boolean> {
  return new Promise((resolve) => {
    const request = http.get("http://127.0.0.1:5173/.game-polish-lab/live-style/farm-slot.json", (response) => {
      response.resume();
...
<body>
  ...
  <script nonce="${nonce}">
    ...
    function updateActionLabels(){const connected=directApplyConnected();const canConnect=adapterId==="idle_monster_farm"&&surfaceType!=="asset_replacement";saveButton.textContent=surfaceType==="asset_replacement"?"Save Asset Metadata":connected?"Save":canConnect?"Save & Connect":"Save Config";saveButton.title=isRuntimeSlotMode()?"Save writes the selected Farm Slot values into the persistent style config/module. The browser game remains the preview.":connected?"Connected direct apply will update the real workspace style path.":canConnect?"Save writes the lab config, then attempts supported runtime setup/connect before returning.":"Saved config only. Direct apply is not connected for this workspace.";const showSetup=canConnect&&!connected;setupButton.textContent=isRuntimeSlotMode()?"Install Runtime Bridge":"One-Time Setup";setupButton.style.display=showSetup||needsSetup?"inline-block":"none";}
    ...
    function scheduleLiveStyleWrite(){if(!isRuntimeSlotMode())return;clearTimeout(liveWriteTimer);liveWriteTimer=setTimeout(()=>{vscode.postMessage({command:"writeLiveStyle",surfaceType,presetName,values,adapterId});},120);}
    ...
    setupButton.addEventListener("click",()=>vscode.postMessage({command:"setupBridge",surfaceType,presetName,values}));
