...
import { AssetReplacementTargetId, BackgroundReadabilityStyleValues, PanelStyleValues, RewardToastStyleValues, SlotCardStyleValues, VisualSurfaceType } from "../types/visualSurface";

type SurfaceValues = SlotCardStyleValues | BackgroundReadabilityStyleValues | PanelStyleValues | RewardToastStyleValues | Record<string, never>;
...
export async function tuneVisualSurface(context: vscode.ExtensionContext): Promise<void> {
  ...
  try {
    ...
    for (const warning of [slotLoad.warning, backgroundLoad.warning, panelLoad.warning, rewardToastLoad.warning].filter((value): value is string => Boolean(value))) {
      logWarn(warning);
    ...
    panel.webview.onDidReceiveMessage(async (message: SaveMessage) => {
      const result = message.command === "applyAsset"
        ...
        ? await setupBridge(folder, message, slotLoad, backgroundLoad, panelLoad, rewardToastLoad)
        : await saveAndApply(folder, message, slotLoad, backgroundLoad, panelLoad, rewardToastLoad);
...
async function saveAndApply(folder: vscode.WorkspaceFolder, message: SaveMessage, slotLoad: StyleConfigLoadResult, backgroundLoad: BackgroundStyleConfigLoadResult, panelLoad: PanelStyleConfigLoadResult, rewardToastLoad: RewardToastStyleConfigLoadResult): Promise<SaveResultMessage> {
  try {
...
async function saveConfigAndApply(
  ...
  fileName: string,
  load: StyleConfigLoadResult | BackgroundStyleConfigLoadResult | PanelStyleConfigLoadResult | RewardToastStyleConfigLoadResult,
  apply: () => Promise<string[]>
): Promise<SaveResultMessage> {
  const scope = checkV05VisualScope([configRelativePath], { throughAdapter: false });
  if (!scope.ok) {
    const error = `v0.55 scope guard blocked config save: ${scope.forbiddenFiles.join(", ")}`;
    logWarn(error);
  ...
  logChecklist(`v0.55 ${surfaceType} manual test checklist:`, checklist);
  return {
    command: "saveResult",
    ok: true,
    surfaceType,
...
async function setupBridge(folder: vscode.WorkspaceFolder, message: SaveMessage, slotLoad: StyleConfigLoadResult, backgroundLoad: BackgroundStyleConfigLoadResult, panelLoad: PanelStyleConfigLoadResult, rewardToastLoad: RewardToastStyleConfigLoadResult): Promise<SaveResultMessage> {
  try {
    if (message.surfaceType === "background_readability") {
...
function checklistFor(surfaceType: Exclude<VisualSurfaceType, "asset_replacement">, load: StyleConfigLoadResult | BackgroundStyleConfigLoadResult | PanelStyleConfigLoadResult | RewardToastStyleConfigLoadResult, rollbackCreated: boolean, applySummary: string[]): string[] {
...
<head>
  ...
  <title>Game Polish Lab v0.55</title>
  <style nonce="${nonce}">
...
<body>
  ...
  <script nonce="${nonce}">
    ...
    const colors={slot_card:["fillColor","borderColor"],background_readability:["backgroundColor","contrastOverlayColor"],panel:["fillColor","borderColor","headerAccentColor","dividerColor"],reward_toast:["toastFillColor","toastBorderColor"]};
