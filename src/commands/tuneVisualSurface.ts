...
<body>
  ...
  <script nonce="${nonce}">
    ...
    function updateActionLabels(){const connected=directApplyConnected();const canConnect=adapterId==="idle_monster_farm"&&surfaceType!=="asset_replacement";saveButton.textContent=surfaceType==="asset_replacement"?"Save Asset Metadata":connected?"Save & Apply":canConnect?"Save & Connect":"Save Config";saveButton.title=connected?"Connected direct apply will update the real workspace style path.":canConnect?"Save writes the lab config, then attempts supported runtime setup/connect before returning.":"Saved config only. Direct apply is not connected for this workspace.";const showSetup=canConnect&&!connected;setupButton.style.display=showSetup||needsSetup?"inline-block":"none";}
