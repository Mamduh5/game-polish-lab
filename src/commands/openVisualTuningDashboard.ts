...
import { getIdleMonsterFarmBackgroundAdapterState } from "../adapters/idleMonsterFarm/backgroundAdapter";
import { getIdleMonsterFarmButtonAdapterState } from "../adapters/idleMonsterFarm/buttonAdapter";
...
import { getIdleMonsterFarmPanelAdapterState } from "../adapters/idleMonsterFarm/panelAdapter";
import { getIdleMonsterFarmRewardToastAdapterState } from "../adapters/idleMonsterFarm/rewardToastAdapter";
  ...
  try {
    ...
    panel.webview.onDidReceiveMessage(async (message: DashboardMessage) => {
      try {
        ...
        await panel.webview.postMessage({ ok: false, message: `Refresh/action failed: ${errorToMessage(error)}` });
      }
    });
...
async function handleDashboardMessage(context: vscode.ExtensionContext, folder: vscode.WorkspaceFolder, model: VisualTuningDashboardModel, message: DashboardMessage): Promise<{ ok: boolean; message: string; refresh?: boolean }> {
  ...
  if (message.command === "directApply") {
    if (row.adapterId === "idle_monster_farm" && row.surfaceType !== "asset_replacement") {
      await tuneVisualSurface(context, { adapterId: "idle_monster_farm", surfaceType: row.surfaceType, targetLabel: row.targetLabel });
      return { ok: true, message: "Idle Monster Farm Runtime Editor opened. Use separate Check Connection, Install Runtime Bridge, Create Baseline Config, Save Style, and Live Edit actions.", refresh: false };
    }
...
async function directApplyFromDashboard(folder: vscode.WorkspaceFolder, row: VisualTuningDashboardRow): Promise<{ ok: boolean; message: string; refresh?: boolean }> {
  if (row.adapterId === "idle_monster_farm" && row.surfaceType !== "asset_replacement") {
    return { ok: false, message: "Idle Monster Farm dashboard direct apply is disabled for runtime style surfaces. Open the Runtime Editor and use separate actions." };
  }
  ...
  if (createdDefaultConfig) {
    ...
    if (!configWriteResult.ok) {
      return { ok: false, message: `Config creation could not write ${row.configPath}: ${configWriteResult.errors.join(" ")}` };
    }
  }
...
async function applyIdleStyleConfig(folder: vscode.WorkspaceFolder, surfaceType: Exclude<VisualSurfaceType, "asset_replacement">, configText: string): Promise<{ ok: boolean; message: string; presetName?: string; styleSnapshot?: object; changedFiles: string[]; rollbackPaths: string[]; checklist: string[]; warnings: string[] }> {
  ...
  void surfaceType;
  void configText;
  return failedDirectApply("Idle Monster Farm dashboard direct apply is disabled for runtime style surfaces. Use the Runtime Editor separate actions.");
}
...
function adapterInfoFromState(adapterId: "idle_monster_farm", state: {
  ...
  connection: { connected: boolean; runtimeProof?: import("../core/visualRuntimeConnectionProof").VisualRuntimeConnectionProof };
...
<body>
  ...
  <script nonce="${nonce}">
    ...
    (model.fieldNotes.knownGood.length?model.fieldNotes.knownGood:["No previous tuning results marked good yet. Mark a result after visual review."]).forEach(v=>li(document.getElementById("good"),v));
    (model.fieldNotes.knownBad.length?model.fieldNotes.knownBad:["No previous tuning results marked bad yet. Keep rollback available after each apply."]).forEach(v=>li(document.getElementById("bad"),v));
    (model.fieldNotes.mixed.length?model.fieldNotes.mixed:["No mixed tuning results recorded yet. Use notes after comparing before and after."]).forEach(v=>li(document.getElementById("mixed"),v));
    ...
    function render(){rows.textContent="";const filtered=model.rows.filter(visible);if(!filtered.length){const empty=document.createElement("article");empty.className="card";const reason=!model.summary.phaserDetected?"No Phaser project detected. Open a Phaser workspace, then run Initialize Project Profile or Run Phaser Pixel Audit.":model.summary.detectedAdapter==="unknown"?"No adapter detected. Use Generic Phaser rows or create a scoped fallback task only after choosing exact visual owner files.":"No surfaces match the current filter. Change the adapter filter or refresh after generating configs/contracts.";empty.innerHTML='<h2>No Visual Surfaces Available</h2><p class="meta">'+reason+'</p><p class="meta">Safe next step: tune a known surface or refresh detection; do not edit gameplay files manually.</p>';rows.append(empty);return;}filtered.forEach(row=>{const card=document.createElement("article");card.className="card";const proof=row.runtimeConnectionProof;card.innerHTML='<div class="row-head"><div><h2>'+row.displayName+'</h2><div class="meta">'+row.surfaceType+' | '+row.adapterId+' | '+row.targetLabel+'</div></div><div class="badges"><span class="badge '+row.appliedStatus+'">'+row.appliedStatus+'</span><span class="badge '+row.lastResult+'">result: '+row.lastResult+'</span><span class="badge">proof: '+(proof?proof.proofLevel:'none')+'</span><span class="badge">template: '+(row.directApplyTemplate.available?(row.directApplyTemplate.executable?'ready':'guarded'):'none')+'</span><span class="badge">warnings: '+row.warningCount+'</span><span class="badge">fallbacks: '+row.fallbackTaskCount+'</span></div></div><div class="grid"><div><b>Config</b><p class="meta">'+(row.configPath||'none')+' ('+row.configStatus+')</p></div><div><b>Recipe</b><p class="meta">'+(row.recipeId||'none')+' ('+row.recipeStatus+')</p></div><div><b>Runtime Proof</b><p class="meta">'+proofSummary(proof)+'</p></div><div><b>Evidence</b><p class="meta">'+proofEvidence(proof)+'</p></div><div><b>Template</b><p class="meta">'+(row.directApplyTemplate.templateId||'none')+' | warn '+row.directApplyTemplate.warningCount+' | block '+row.directApplyTemplate.blockCount+'</p></div><div><b>Last Tuned</b><p class="meta">'+(row.lastTunedAt||'No previous tuning result. Tune by eye, save config, then mark the result.')+'</p></div></div><p class="meta">'+(row.latestNoteSummary||row.directApplyTemplate.reason||'')+'</p>';const actions=document.createElement("div");actions.className="actions";for(const [command,action] of Object.entries(row.actions)){const button=document.createElement("button");button.textContent=action.label;button.disabled=!action.enabled&&command!=="openConfig";button.className=command==="tune"?"":"secondary";button.title=action.reason||action.label;button.addEventListener("click",()=>vscode.postMessage({command,rowId:row.rowId}));actions.append(button);}card.append(actions);rows.append(card);});}
    function proofSummary(proof){if(!proof)return'No runtime proof model for this row.';return 'Direct apply: '+(proof.connected?'connected':'not connected')+' | status '+proof.status+' | style source '+proof.styleSource+' | enabled by runtime usage '+(proof.connected&&proof.proofLevel==='runtime_value_usage'?'yes':'no');}
    function proofEvidence(proof){if(!proof)return'none';const evidence=(proof.evidenceFiles||[]).slice(0,3).map(e=>e.relativePath+' '+e.evidenceKind+(e.matchedProperties&&e.matchedProperties.length?' ['+e.matchedProperties.join(', ')+']':''));const missing=(proof.missingPieces||[]).slice(0,2).map(m=>'Missing: '+m);return evidence.concat(missing).join(' | ')||'none';}
    function post(command,rowId,button){if(command==="refresh"||command==="refreshAssetContracts"){if(button)button.disabled=true;status.textContent=command==="refresh"?"Refreshing dashboard...":"Refreshing asset contracts...";}vscode.postMessage({command,rowId});}
    filter.addEventListener("change",render);document.getElementById("refresh").addEventListener("click",event=>post("refresh",undefined,event.currentTarget));document.getElementById("refreshAssetContracts").addEventListener("click",event=>post("refreshAssetContracts",undefined,event.currentTarget));document.getElementById("openRollbackHistory").addEventListener("click",()=>post("openRollbackHistory"));document.getElementById("openAssetContactSheet").addEventListener("click",()=>post("openAssetContactSheet"));document.querySelectorAll("[data-global]").forEach(b=>b.addEventListener("click",()=>post(b.dataset.global)));window.addEventListener("message",event=>{const m=event.data;status.textContent=(m.ok?'OK: ':'Blocked: ')+m.message;document.querySelectorAll("button:disabled").forEach(button=>{if(button.id==="refresh"||button.id==="refreshAssetContracts")button.disabled=false;});});render();
  </script>
</body>
</html>`;
}
