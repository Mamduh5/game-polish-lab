  ...
  try {
    ...
    panel.webview.onDidReceiveMessage(async (message: AssetPipelineMessage) => {
      try {
        ...
        await panel.webview.postMessage({ ok: false, message: `Asset dashboard action failed: ${errorToMessage(error)}` });
      }
    });
...
async function handleAssetPipelineMessage(folder: vscode.WorkspaceFolder, model: VisualAssetDashboardModel, message: AssetPipelineMessage): Promise<{ ok: boolean; message: string; refresh?: boolean }> {
  ...
  if (message.command === "openManifestContract") {
    if (!row.manifestContract) {
      return { ok: false, message: "No manifest contract is available for this slot." };
    }
    const assetContractUri = vscode.Uri.joinPath(folder.uri, ...visualAssetContractRelativePath.split("/"));
    if (await pathExists(assetContractUri)) {
      await openTextDocument(assetContractUri);
      return { ok: true, message: `Opened ${visualAssetContractRelativePath}.` };
    }
    if (row.manifestContract.manifestPath) {
      const manifestUri = vscode.Uri.joinPath(folder.uri, ...row.manifestContract.manifestPath.split("/"));
      if (await pathExists(manifestUri)) {
        await openTextDocument(manifestUri);
        return { ok: true, message: `Opened ${row.manifestContract.manifestPath}.` };
      }
    }
    return { ok: true, message: `Manifest contract ${row.manifestContract.contractId}: ${row.manifestContract.writablePathSafety}.` };
  }
  if (message.command === "openManifestApplyResult") {
    if (!row.manifestApplyResult?.operationId) {
      return { ok: false, message: "No manifest apply result exists for this slot." };
    }
    const relativePath = `${visualAssetManifestApplyRelativeDir}/${row.manifestApplyResult.operationId}.json`;
    await openTextDocument(vscode.Uri.joinPath(folder.uri, ...relativePath.split("/")));
    return { ok: true, message: `Opened ${relativePath}.` };
  }
  if (message.command === "applyManifestAssignment") {
    if (!row.manifestContract) {
      return { ok: false, message: "No manifest contract is available for this slot." };
    }
    const result = applyVisualAssetManifestAssignment({
      ...
      contract: row.manifestContract
    });
    return {
      ok: result.status === "applied",
      message: `Manifest apply ${result.status}. Runtime applied: ${result.runtimeApplied ? "yes" : "no"}. Files: ${result.filesWritten.join(", ") || "none"}. Warnings: ${result.warnings.join(" | ") || "none"}. Errors: ${result.errors.join(" | ") || "none"}.`,
      refresh: true
    };
  }
  ...
  if (message.command === "generateLoaderFallbackTask") {
    ...
    const task = buildManifestLoaderFallbackTask({
      ...
      validation: row.validation,
      boundsAnalysis: row.boundsAnalysis,
      styleGuidePath: row.styleGuide?.markdownPath,
      contract: row.manifestContract,
      reason: row.manifestContract
        ? `${row.manifestContract.contractId} is ${row.manifestContract.writablePathSafety}; ${row.manifestContract.errors.concat(row.manifestContract.warnings).join(" ")}`
        : "No manifest contract is available."
    });
    const taskPath = writeManifestLoaderFallbackTask(folder.uri.fsPath, task);
    return { ok: true, message: `Created scoped loader fallback task: ${taskPath}.`, refresh: true };
  }
...
<head>
  ...
  <style nonce="${nonce}">
    ...
    *{box-sizing:border-box}body{margin:0;padding:18px;color:var(--text);font-family:var(--vscode-font-family);background:var(--vscode-editor-background)}h1,h2,h3,p{margin:0}h1{font-size:21px}h2{font-size:15px}.top{display:grid;grid-template-columns:1fr auto;gap:12px;align-items:start;margin-bottom:14px}.summary{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:8px;margin:12px 0}.metric,.card,.preview{border:1px solid color-mix(in srgb,var(--border) 72%,transparent);background:var(--panel);border-radius:8px;padding:12px}.metric b{display:block;font-size:17px}.muted,.meta{color:var(--muted);font-size:12px;line-height:1.45}.toolbar,.actions{display:flex;gap:7px;align-items:center;flex-wrap:wrap}.surface{display:grid;gap:10px;margin-top:14px}.card{display:grid;gap:10px}.row-head{display:grid;grid-template-columns:1fr auto;gap:12px}.badges{display:flex;flex-wrap:wrap;gap:6px}.badge{border:1px solid color-mix(in srgb,var(--border) 68%,transparent);border-radius:999px;padding:3px 8px;font-size:12px;color:var(--muted);background:color-mix(in srgb,var(--panel) 82%,var(--vscode-editor-background))}.valid,.manifest_supported{color:var(--vscode-testing-iconPassed)}.warning,.fallback_required{color:var(--warn);border-color:color-mix(in srgb,var(--warn) 45%,transparent)}.invalid,.unsupported{color:var(--error);border-color:color-mix(in srgb,var(--error) 45%,transparent)}.config_only,.asset_copy_only{color:var(--vscode-charts-blue)}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:7px}button{min-height:28px;color:var(--button-text);background:var(--button);border:1px solid transparent;border-radius:4px;padding:3px 10px;transition:background-color .12s ease,border-color .12s ease,transform .08s ease,opacity .12s ease}button:hover:not(:disabled){background:var(--vscode-button-hoverBackground);border-color:color-mix(in srgb,var(--focus) 45%,transparent)}button:focus-visible{outline:1px solid var(--focus);outline-offset:2px}button:active:not(:disabled){transform:translateY(1px)}button:disabled{opacity:.52;cursor:not-allowed}.secondary{color:var(--vscode-button-secondaryForeground);background:var(--vscode-button-secondaryBackground)}.secondary:hover:not(:disabled){background:var(--vscode-button-secondaryHoverBackground)}img{max-width:96px;max-height:96px;image-rendering:pixelated;border:1px solid color-mix(in srgb,var(--border) 70%,transparent);background:var(--vscode-editor-background)}.status{white-space:pre-wrap;margin-top:12px}@media(max-width:720px){.top,.row-head{grid-template-columns:1fr}}
  </style>
</head>
<body>
