  ...
  try {
    ...
    panel.webview.onDidReceiveMessage(async (message: RollbackHistoryMessage) => {
      if (message.command === "refresh") {
        statusMessage = "Rollback history refreshed.";
        render();
        return;
      }
...
function renderRollbackHistoryHtml(snapshots: VisualRollbackSnapshot[], warnings: string[], statusMessage: string): string {
  ...
  const payload = JSON.stringify({ snapshots, warnings, statusMessage }).replace(/</g, "\\u003c");
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  ...
  <style nonce="${nonce}">
    ...
    *{box-sizing:border-box}body{margin:0;padding:18px;color:var(--text);font-family:var(--vscode-font-family);background:var(--vscode-editor-background)}h1,h2,p{margin:0}h1{font-size:21px}h2{font-size:15px}.top{display:grid;grid-template-columns:1fr auto;gap:12px;align-items:start;margin-bottom:14px}.meta,.status{color:var(--muted);font-size:12px;line-height:1.45}.rows{display:grid;gap:10px}.card{border:1px solid var(--border);background:var(--panel);border-radius:8px;padding:12px;display:grid;gap:10px}.head{display:grid;grid-template-columns:1fr auto;gap:12px}.badges,.actions{display:flex;flex-wrap:wrap;gap:7px}.badge{border:1px solid var(--border);border-radius:999px;padding:2px 8px;font-size:12px;color:var(--muted)}.safe{color:#89d185}.warn{color:#dcdcaa}.block{color:#f48771}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:7px}.file{border-top:1px solid var(--border);padding-top:8px}button{min-height:28px;color:var(--button-text);background:var(--button);border:1px solid transparent;border-radius:4px;padding:3px 10px}button:disabled{opacity:.45}.secondary{color:var(--vscode-button-secondaryForeground);background:var(--vscode-button-secondaryBackground)}.empty{border:1px solid var(--border);padding:14px;border-radius:8px}.status{white-space:pre-wrap;margin-top:12px}@media(max-width:720px){.top,.head{grid-template-columns:1fr}}
  </style>
</head>
<body>
  ...
  <script nonce="${nonce}">
    ...
    function render(){rows.textContent="";status.textContent=[data.statusMessage||"",...(data.warnings||[]).map(w=>"warning: "+w)].filter(Boolean).join("\\n");if(!data.snapshots.length){const empty=document.createElement("div");empty.className="empty meta";empty.textContent="No rollback snapshots found.";rows.append(empty);return;}for(const snapshot of data.snapshots){const c=counts(snapshot);const card=document.createElement("article");card.className="card";card.innerHTML='<div class="head"><div><h2>'+snapshot.id+'</h2><p class="meta">'+text(snapshot.createdAt)+' | '+text(snapshot.adapterId)+' | '+text(snapshot.surfaceType)+' | '+text(snapshot.targetId||snapshot.targetLabel)+'</p></div><div class="badges"><span class="badge safe">safe '+c.safe+'</span><span class="badge warn">warn '+c.warn+'</span><span class="badge block">blocked '+c.block+'</span><span class="badge">files '+snapshot.files.length+'</span></div></div>';const grid=document.createElement("div");grid.className="grid";for(const file of snapshot.files){const item=document.createElement("div");item.className="file";item.innerHTML='<b>'+file.fileKind+'</b><p class="meta">'+file.originalPath+'</p><p class="meta">snapshot: '+file.snapshotPath+'</p><p class="meta">'+file.scopeClassification.classification+' | '+file.scopeClassification.reasonCode+'</p><p class="meta">'+(file.restoreEligible?'eligible':'fallback/block required')+'</p>';grid.append(item);}card.append(grid);const actions=document.createElement("div");actions.className="actions";const restore=document.createElement("button");restore.textContent=c.eligible>0?"Restore Eligible Files":"Create Fallback / Inspect";restore.disabled=snapshot.files.length===0;restore.title=c.eligible>0?"Restore safe visual files only":"No safe auto-restore files; a fallback task may be created for suspicious files";restore.addEventListener("click",()=>vscode.postMessage({command:"restore",snapshotId:snapshot.id}));actions.append(restore);card.append(actions);rows.append(card);}}
    document.getElementById("refresh").addEventListener("click",()=>vscode.postMessage({command:"refresh"}));render();
  </script>
</body>
</html>`;
}
