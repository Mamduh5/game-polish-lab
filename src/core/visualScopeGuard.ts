...
const safeRules: PathRule[] = [
  ...
  safe("explicit_manifest_contract_path", "Explicit manifest contract marks this visual-asset-only path safe for manifest direct apply.", (path, request) => request.operationType === "asset_manifest_direct_apply" && !/\.(ts|tsx|js|jsx)$/.test(path) && (request.explicitSafePaths ?? []).map(normalizeVisualScopePath).includes(path)),
  ...
  safe("asset_file", "Asset files in known asset folders are safe visual targets.", (path, request) => request.operationType !== "asset_pipeline_assignment" && request.operationType !== "asset_style_guide_generation" && request.operationType !== "asset_manifest_direct_apply" && /^(src\/assets|public\/assets|assets)\//.test(path) && !includesAny(path, ["manifest", "registry", "loader"])),
